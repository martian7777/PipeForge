...
- **JWT bearer tokens** (HS256), issued on register/login, verified on every protected
  request via the `current_user` dependency (`app/security.py`). Tokens are stateless â€”
  no server-side session â€” so the fleet scales without sticky sessions.
...
- Configure via env: `PIPEFORGE_JWT_SECRET` (**must** be a â‰¥32-byte random string in prod),
  `PIPEFORGE_ACCESS_TOKEN_TTL_MINUTES`.
...
- All request bodies are **Pydantic v2** models with `extra="forbid"` â€” unknown fields
...
- Storage backend is configurable: in-memory for a single process, or **Redis**
  (`PIPEFORGE_RATELIMIT_STORAGE=redis://...`) so limits are enforced **across all replicas**.
...
- **CORS** is restricted to configured origins (`PIPEFORGE_CORS_ORIGINS`). In the Docker
...
- [ ] Set a strong `PIPEFORGE_JWT_SECRET` and rotate periodically.
