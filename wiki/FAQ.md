...
Not yet — Milestones 1–2 (ingest, clean, EDA) are complete. Model training, the
...
configurable via `AUTODS_ACCESS_TOKEN_TTL_MINUTES`).

### I get `429 Too Many Requests`.
You hit a rate limit (defaults: 200/min general, 10/min auth, 30/min upload). Wait and
retry, or raise the limits via `AUTODS_RATELIMIT_*` env vars.

### My upload fails with `413`.
The file exceeds the 200 MB cap (`AUTODS_MAX_UPLOAD_BYTES`). Raise it or split the file.

### VS Code shows thousands of pending git changes.
That's a stale view from before `.gitignore` existed. `.venv/`, `node_modules/`, and
build artifacts are all ignored — `git status` shows only real source files. Reload the
window or run `git status` to confirm.

### How do I switch from SQLite to Postgres?
Set `AUTODS_DATABASE_URL=postgresql+psycopg://user:pass@host:5432/db`. The Docker compose
...
errored, no report exists — check `GET /api/runs/{id}/status` for the error message.
